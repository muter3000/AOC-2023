...
positions = [(start,start) for start in re.findall('[A-Z0-9][A-Z0-9]A',joined)]
x = 0
while not all([isEnd(c) for (_,c) in positions]):
    x+=1
    for i,(s, current) in enumerate(positions):
        positions[i] = (s, translated_graph[current])
